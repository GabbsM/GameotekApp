...
import com.gaby.gameotekaapp.presentation.screens.desarrollador.DesarrolladorContent

@Composable
fun DistribuidorScreen(navController: NavHostController, viewModel: RawGViewModelList) {

    Scaffold(
        topBar = {  DefaultTopBar(tittle = "Distribuidor", upAvailable = false, navController = navController,color= Color.White) },
        content = { paddingValues ->
            Column {
                Spacer(modifier = Modifier.height(56.dp)) // Añade un espacio de 56.dp que es la altura típica de una TopBar

                DistribuidorContent(paddingValues = paddingValues, navHostController = navController, viewModel = viewModel)
            }
        },
